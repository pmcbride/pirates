...
3. **Predict-then-run.** Before playback starts (every mission except the
   ...
   ship end up?" The player drops a marker on a tile, and the dock's hex Play
   button — disabled until the marker lands, then pulsing ready — confirms the
   run. The whole beat lives in the command dock (head copy + "Skip prediction"
   link); nothing floats over the board the child is being asked to tap. After
   playback the reward (or hint) screen reports whether the prediction was
   right — converting passive watching into active reasoning per
   Wing/Bers/Resnick research on early CT. The skip choice persists in the
   profile for parents who want to opt out.
...
|               | loader; missions fall back to the procedural colored stamps (with   |
|               | their 2-letter captions) when a file fails to load.                 |
